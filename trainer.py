...
def validate_in_test(args, val_loader, model, DtoD_model,epoch, logger,mode='DtoD',crop_mask = None,criterion_L2=None):
    ...
    for i, (depth, img, depth_np) in enumerate(val_loader):
        ...
        if depth_np is not None:
            sparse_depths = depth_np.to(device)
            sparse_depths = Variable(sparse_depths)

        origin = depths.to(device)
        ...
        ################# gradient loss #######################
        grad_latent_loss = torch.tensor(0.).to(device)
        if args.mode != 'RtoD_single':
...
def train_AE_DtoD(args,model,criterion_L2,criterion_L1, optimizer, dataset_loader,val_loader, batch_size, n_epochs,lr,logger, train_writer):
    global n_iter, best_error, device
    ...
    depth_fixed = depth_fixed.to(device)
    rgb_fixed = rgb_fixed.to(device)
    ...
    W = depth_fixed.shape[2]
    H = depth_fixed.shape[3]
    ...
    if args.dataset != 'NYU':
        ...
        ftmap_height_list = [H, int(H / 3), int(H / 5), int(H / 9), int(H / 15), H]
        ftmap_width_list = [W, int(W / 3), int(W / 6), int(W / 11), int(W / 22), W]
    else:
