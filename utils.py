...
def save_image_tensor(tensor_img,img_dir,filename):
    ...
    if(tensor_img.shape[1]==1):
        ...
        exit()
    imageio.imwrite(img_dir + '/' + filename,input__)
...
def save_image_batch(model,rgb_fixed, depth_fixed, predicted_dirs, num):
    ...
    with torch.no_grad():
        ...
        # outputs_fixed_ = torch.empty([B,1,H,W]).to(device)
        # depth_fixed_ = torch.empty([B,1,H,W]).to(device)
        # for r in range(3):
        #     outputs_fixed_[:,r,:,:] = outputs_fixed[:,0,:,:]
        #     depth_fixed_[:,r,:,:] = depth_fixed[:,0,:,:]
        img_list.append(outputs_fixed)
        img_list.append(depth_fixed)
