...
def main():
    ...
    if args.mode == 'DtoD':
        ...
        AE_DtoD = AE_DtoD.to(device)
        # AE_DtoD.load_state_dict(torch.load(args.model_dir))
        print('- DtoD model is created')
        optimizer_AE = optim.Adam(AE_DtoD.parameters(), args.lr, [args.momentum, args.beta], eps=1e-08,
                                  weight_decay=5e-4)
        ...
        optimizer_AE = optim.Adam(AE_RtoD.parameters(), args.lr, [args.momentum, args.beta], eps=1e-08,
                                  weight_decay=5e-4)
        criterion_L2 = nn.MSELoss()
        criterion_L1 = nn.L1Loss()
    ...
    ############################ Training part ##################################
    if args.mode == 'DtoD':
        final = train_AE_DtoD(args, AE_DtoD, criterion_L2, criterion_L1, optimizer_AE, train_loader, val_loader,
                              args.batch_size, args.epochs, args.lr, logger, training_writer)
        print('Final loss:', final.item())
        loss = final
    elif args.mode == 'RtoD' or args.mode == 'RtoD_single':
        loss, output_loss, latent_loss = train_AE_RtoD(args, AE_RtoD, AE_DtoD, criterion_L2, criterion_L1, optimizer_AE,
                                                       train_loader,
    ...
    if (logger is not None) and (args.evaluate == True):
        ...
        error_string = ', '.join('{} : {:.3f}'.format(name, error) for name, error in
                                 zip(error_names[0:error_length], errors[0:error_length]))
        logger.valid_writer.write(' * Avg {}'.format(error_string))
        print("")
        error_string = ', '.join('{} : {:.3f}'.format(name, error) for name, error in
                                 zip(error_names[0:error_length], min_errors[0:error_length]))
        logger.valid_writer.write(' * Avg {}'.format(error_string))
